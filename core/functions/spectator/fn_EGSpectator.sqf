...
switch _mode do {
	...
	case "onLoad": {
		...
		EGSpectatorLoopPFH = [{
			params ["_args", "_idPFH"];
			if !(uiNamespace getVariable [VAR_INTERFACE_VISIBLE, true]) exitWith {
				[_idPFH] call CBA_fnc_removePerFrameHandler;
			};
			["Update_GameTime"] call DISPLAY;
			["Update_FocusInfoWidget"] call DISPLAY;
		}, LOOP_RATE, []] call CBA_fnc_addPerFrameHandler;
		...
		{
			(_display displayCtrl _x) ctrlAddEventHandler ["ButtonClick", { ["CamButtonClicked", _this] call DISPLAY; }];
		...
		if !(missionNamespace getVariable [VAR_ALLOW_FREE_CAMERA, true]) then {
			...
			if (!isNull _initialFocus) then {
				...
				// If 3PP mode is allowed we prefer that one
				// Otherwise we force FPS mode
				if !(missionNamespace getVariable [VAR_ALLOW_3PP_CAMERA, true]) then {
		...
		if (!(missionNamespace getVariable [VAR_ALLOW_FREE_CAMERA, true]) && {!(missionNamespace getVariable [VAR_ALLOW_3PP_CAMERA, true])}) then {
			// Override since we can't change to any other camera, we might as well just disable the widget all together
			missionNamespace setVariable [VAR_SHOW_CAMERA_BUTTONS, false];
		} else {
			// Set free camera button to fully disabled state if not available
			if !(missionNamespace getVariable [VAR_ALLOW_FREE_CAMERA, true]) then {
			...
			// Set follow camera button to fully disabled state if not available
			if !(missionNamespace getVariable [VAR_ALLOW_3PP_CAMERA, true]) then {
				(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_FOLLOW) ctrlSetTextColor [1,1,1,0.2];
				(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_FOLLOW) ctrlSetTooltip "";
			};

			// Set current selected camera button
			switch (["GetCameraMode"] call CAM) do {
		...
		// Header
		if !(missionNamespace getVariable [VAR_SHOW_HEADER, true]) then {(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_GAMETIMETEXT) ctrlShow false};

		// Camera buttons
		if !(missionNamespace getVariable [VAR_SHOW_CAMERA_BUTTONS, true]) then {(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_CAMERATYPESGROUP) ctrlShow false};
		...
		if !(missionNamespace getVariable [VAR_SHOW_LISTS, true]) then {(_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_LIST) ctrlShow false; (_display displayCtrl IDC_RSCDISPLAYEGSPECTATOR_TABS) ctrlShow false};
		...
		["SetControlsHelpVisibility", [missionNamespace getVariable [VAR_SHOW_CONTROLS_HELPER, true]]] call DISPLAY;
	...
	case "onUnload": {
		...
		// Stop our loop
		[EGSpectatorLoopPFH] call CBA_fnc_removePerFrameHandler;
		...
		if (!isNil { uiNamespace getVariable VAR_KEY_DOWN } && !isNil { uiNamespace getVariable VAR_DISPLAY } && { !isNull (uiNamespace getVariable VAR_DISPLAY) }) then {
			(uiNamespace getVariable [VAR_DISPLAY, displayNull]) displayRemoveEventHandler ["KeyDown", uiNamespace getVariable [VAR_KEY_DOWN, -1]];
		};
		...
		if (!isNil { uiNamespace getVariable VAR_KEY_UP } && !isNil { uiNamespace getVariable VAR_DISPLAY } && { !isNull (uiNamespace getVariable VAR_DISPLAY) }) then {
			(uiNamespace getVariable [VAR_DISPLAY, displayNull]) displayRemoveEventHandler ["KeyUp", uiNamespace getVariable [VAR_KEY_UP, -1]];
		};
	...
	case "Update_GameTime" : {
		...
		if !(missionNamespace getVariable [VAR_SHOW_HEADER, true]) exitWith {};
	...
	case "Update_LocationList" : {
		...
		if !(_oldList isEqualTo _newList) then {
...

			{
				...
				private _i = ["TreeGetDataIndex", [_id]] call DISPLAY;
	...
	case "Update_FocusInfoWidget" : {
		...
		if (!isNull _focus && {missionNamespace getVariable [VAR_SHOW_FOCUS_INFO, true]} && {uiNamespace getVariable [VAR_FOCUS_WIDGET_VISIBLE, false]}) then {
			...
			private ["_name", "_unitIcon", "_unitTypePicture", "_weapon", "_insigniaTexture"];
			...
			_unitIcon = getText (configfile >> "CfgVehicles" >> typeOf _focus >> "icon");
			_unitTypePicture = if (_unitIcon != "") then {getText (configfile >> "CfgVehicleIcons" >> _unitIcon)} else {""};
	...
	case "SetFocus" : {
		...
		if (_newFocus != _oldFocus && { !isNull _newFocus || { !isNull _oldFocus } }) then {
			...
			//AK TANK MODE: if we selected the same focus as the previous one, just make the camera look at it
			if(!isNull _newFocus && {missionNamespace getVariable ["BIS_AK_vanguardSpectator", false]}) then {
	...
	case "OnFocusChanged" : {
		...
		// Select in tree
		if (!isNull _newFocus) then {
			["TreeSelectFromData", [[_newFocus] call BIS_fnc_objectVar]] call DISPLAY;
		} else {
			["TreeUnselect"] call DISPLAY;
		};
	};
	...
	case "KeyDown": {
		...
		switch true do {
			...
			case (_key == DIK_RIGHT): 		{ ["OnSwitchFocusClicked", [true]] call DISPLAY; true; };
			case (_key == DIK_LEFT): 		{ ["OnSwitchFocusClicked", [false]] call DISPLAY; true; };
			case (_key == DIK_N): 			{ ["ToggleVisionMode"] call CAM; true; };
			case (_key == DIK_SPACE): 		{ ["ToggleCameraMode"] call CAM; true; };
			case (_key == DIK_NUMPADENTER): { ["ToggleCameraMode"] call CAM; true; };
			case (_key == DIK_L): 			{ ["ToggleCameraLights"] call CAM; true; };
			case (_key == DIK_LALT): 		{ ["ToggleVeryFastSpeed", [true]] call CAM; true; };
			case (_key == DIK_O): 			{ missionNamespace setVariable [VAR_DRAW_PROJECTILES_PATH, !(missionNamespace getVariable [VAR_DRAW_PROJECTILES_PATH, false])]; true; };
			default 						{ false; };
		};
	};
	...
	case "Interrupt" : {
		if (uiNamespace getVariable [VAR_MAP_VISIBLE, false]) then {
	...
	case "DrawMap" : {
		...
		if (!isNull _map) then {
			...
			if (!isNull _display && !isNull _camera) then {
				...
				_entities = ["GetTargetEntities"] call SPEC;
	...
	case "OnSwitchFocusClicked" : {
		private _next = _params param [0, true, [true]];
		private _entities = ["GetTargetEntities"] call SPEC;
		private _focus = ["GetFocus"] call DISPLAY;

		if (count (_entities - [_focus]) < 1) exitWith
		{
			"OnSwitchFocusClicked: Unable to cycle focus" call BIS_fnc_log;
		};

		private _index = _entities find _focus;
		if (_index < 0) then {_index = 0};

		if (_next) then {
			_index = _index + 1;

			if (_index > count _entities - 1) then {
				_index = 0;
			};
		} else {
			_index = _index - 1;
			...
			if (_index < 0) then {
				_index = count _entities - 1;
			};
		};

		if (_index >= 0 && {_index <= count _entities - 1}) then {
			["SetFocus", [_entities select _index]] call DISPLAY;
		};
	};
	...
	case "OnMapClicked" : {
		...
		if (!isNull _camera) then {
			if (!isNull _highlightedObject) then {
				["SetFocus", [_highlightedObject]] call DISPLAY;
		...
		switch (_key) do {
			...
			case 0 : {
				...
				if (!isNull _cursorObject) then {
					["SetFocus", [_cursorObject]] call DISPLAY;
					playsound "ReadoutClick";
			...
			if (!isNull _control) then {
				for "_i" from 0 to ((_control tvCount []) - 1) do {
					for "_a" from 0 to ((_control tvCount [_i]) - 1) do {
						...
						{
							// Delete units
							for "_u" from 0 to ((_control tvCount [_i, _a]) - 1) do {
								_control tvDelete [_i, _a, _u];
							};
	...
	case "SetControlsHelpVisibility" : {
		...
		if (_bVisible && {missionNamespace getVariable [VAR_SHOW_CONTROLS_HELPER, true]}) then {
	...
	case "GotoLocation" : {
		...
		private _center = getPosASL _target;
		private _dirTo = vectorDir _target;
		private _dirFrom = _dirTo vectorMultiply -1;
		private _loc = _center vectorAdd (_dirFrom vectorMultiply 50);
		_loc set [2, (_loc select 2) + 10.0];
		private _dir = [_loc, _center] call BIS_fnc_dirTo;

		private _cameraTransform = [_loc, [0,0,0], [0,0,0], [_dir, true]];
