...
if (({(_x select 1) == side player} count GVAR(MissionCalls)) > 0) then {
	...
	for "_i" from 0 to count GVAR(MissionCalls) -1 do {
		...
		if ((_option select 1) == side player) then {
			_html = _html + "<execute expression='[""" + (_option select 0) + """, false] call " + QFUNC(CallMissionReq) + ";'>" + (_option select 2) + "</execute>";
...
if (({(_x select 1) == sideUnknown} count GVAR(MissionCalls)) > 0) then {
	...
	for "_i" from 0 to count GVAR(MissionCalls) -1 do {
		...
		if ((_option select 1) == sideUnknown) then {
			_html = _html + "<execute expression='[""" + (_option select 0) + """, true] call " + QFUNC(CallMissionReq) + ";'>" + (_option select 2) + "</execute>";
			if (_i < count GVAR(MissionCalls)) then {
				_html = _html + "<br/>";
			};
		};
...
[{!isNull ace_player && {player diarySubjectExists QEGVAR(FW,Menu)}},{
	player createDiaryRecord [QEGVAR(FW,Menu), ["Call Mission", _this]];
}, _html] call CBA_fnc_waitUntilAndExecute;
