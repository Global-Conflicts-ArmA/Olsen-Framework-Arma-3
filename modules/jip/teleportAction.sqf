...
if !((_target call FUNC(Alive)) && {(!(_target call FUNC(InVehicle)) || ((vehicle _target) call FUNC(HasEmptyPositions)))}) then {
...

	{
		if (_x call FUNC(Alive)) then {
			_count = _count + 1;
			if ((rankId _x > _rank) && {!(_x call FUNC(InVehicle)) || ((vehicle _x) call FUNC(HasEmptyPositions))}) then {
				_rank = rankId _x;
				_target = _x;
			};
		};
