...
FNC_DIA_DonutFiremissionOpenDialog =
{
	...
	[DFM_DIA_IDC_GUNSELECT,DFM_DIA_IDC_SHELLSELECT] call EFUNC(FIREMIS,Dia_ArtLoadAvailableArtilleries);
};

FNC_DIA_DonutFiremissionSetArtillery =
{
	[DFM_DIA_IDC_SHELLSELECT,_this] call EFUNC(FIREMIS,Dia_ArtSetArtillery);
};
...
FNC_DIA_DonutFiremissionFire =
{
	...
	_inputIsCorrect = _inputIsCorrect && [_innerRadius,"Inner radius is not a number"] call EFUNC(FIREMIS,Dia_InputIsNumber);
	_inputIsCorrect = _inputIsCorrect && [_outerRadius,"Outer radius is not a number"] call EFUNC(FIREMIS,Dia_InputIsNumber);
	...
	if(_inputIsCorrect ) then
	{
								...
								hint (([_selectedUnit,_grid call CBA_fnc_mapGridToPos,_innerRadius,_outerRadius,_burstNumber,_burstRounds,_burstDelay,_spotting,_selectedAmmo] call EFUNC(FIREMIS,GetDonutFiremissionText))
...
FNC_DIA_Server_DonutFiremissionFire =
{
	...
	[_selectedUnit ,[_grid,true] call CBA_fnc_mapGridToPos,_innerRadius,_outerRadius,_burstNumber,_burstRounds,_burstDelay,_spotting,_selectedAmmo]   call EFUNC(FIREMIS,DonutFiremission);
