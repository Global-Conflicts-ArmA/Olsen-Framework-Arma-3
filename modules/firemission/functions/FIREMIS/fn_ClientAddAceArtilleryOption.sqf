...
if (!(player getVariable [VAR_SART_PLAYERRECEIVEDGUNS,false]) && {_guns isNotEqualTo []}) then {
  ...
  [player, 1, ["ACE_SelfActions","Artillery_Menu"], _action] call ace_interact_menu_fnc_addActionToObject;
  _guns apply {
    private _artyName =_x call FUNC(Dia_GetArtyDisplayName);
    private _text = ("Stop " + _artyName);
    _action = ["Stop",_text , "", {(_this select 2) call FUNC(StopArtilleryClient); }, {!(( _this select 2) call FUNC(Dia_IsArtyAvailable))},{},_x] call ace_interact_menu_fnc_createAction;
  ...
  _guns apply {
    ...
    _action = ["Info",_text , "",{hint ((_this select 2) call FUNC(Dia_GetCompleteInfoText)); }, { !((_this select 2) call FUNC(Dia_IsArtyAvailable))},{},_x] call ace_interact_menu_fnc_createAction;
    [player, 1, ["ACE_SelfActions","Artillery_Menu","FiremissionInformation"], _action] call ace_interact_menu_fnc_addActionToObject;
  };

  ["Event_ArtyIsReady",
  {
    [PFM_DIA_IDC_GUNSELECT] call FUNC(Dia_ArtLoadAvailableArtilleries);
    [LFM_DIA_IDC_GUNSELECT] call FUNC(Dia_ArtLoadAvailableArtilleries);
    [BFM_DIA_IDC_GUNSELECT] call FUNC(Dia_ArtLoadAvailableArtilleries);
    [DFM_DIA_IDC_GUNSELECT] call FUNC(Dia_ArtLoadAvailableArtilleries);
    [MFM_DIA_IDC_GUNSELECT] call FUNC(Dia_ArtLoadAvailableArtilleries);
    [PSFM_DIA_IDC_GUNSELECT] call FUNC(Dia_ArtLoadAvailableArtilleries);
    [GSFM_DIA_IDC_GUNSELECT] call FUNC(Dia_ArtLoadAvailableArtilleries);
