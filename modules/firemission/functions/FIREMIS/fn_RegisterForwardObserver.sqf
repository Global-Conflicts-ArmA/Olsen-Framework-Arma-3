									...
									if(alive _observer) then {
										...
										_batteries apply {
												if((!(_x getVariable [VAR_SART_ARTINFIREMISSION,false])) && !(_hasFired) ) then {
														...
														[_x,_pos,_standardDispersion,_standardRoundCount,_standardRoundBurst,_standardRoundBurstWait,_minSpottedDistance,_standardRound] call FUNC(PointFiremission);
														_hasFired = true;
												};
