...
private _handle = _this spawn
{
  ...
  _unit setVariable [VAR_SART_ARTFMTEXT,_this call FUNC(GetPointFiremissionText),true];
  ...
  for "_i" from 0 to _burstCount do
  {
    if(_isForward) then
    {
        [_unit,_startPoint vectorAdd (_dir vectorMultiply (_i / 2)),0,_burstSize,_roundClassName] call FUNC(InternalFiremission);
    ...
    else
    {
        ...
        [_unit,_startPoint vectorAdd (_dir vectorMultiply ((_burstCount - _i) / 2)),0,_burstSize,_roundClassName] call FUNC(InternalFiremission);
        [_unit, ((_unit getVariable [VAR_SART_ARTROUNDSFIRED,[0,0]]) select 0) + _burstSize,_burstCount * _burstSize] call FUNC(Dia_SetArtyFiremissionRoundsRequired);
        _isForward = true;
    };
